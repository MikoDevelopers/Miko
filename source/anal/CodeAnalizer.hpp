...
namespace Analizer {
    class Functions{
        public:
        bool returnToF(std::string input){
             if(input.find("==") != -1){
                std::string LeftDatas = input.substr(0, input.find("==") - 1);
                std::string RightDatas = input.substr(input.find("==")+2, input.size());
                if(std::stoi(LeftDatas) == std::stoi(RightDatas)) return 1;
                else return 0;
            }
                ...
                std::string LeftDatas = input.substr(0, input.find(">") - 1);
                std::string RightDatas = input.substr(input.find(">")+1, input.size());
                if(std::stoi(LeftDatas) > std::stoi(RightDatas)) return 1;
                else return 0;
            }
            else if(input.find("<") != -1){
        ...
        public:
            ...
            int findIEE_Foo(std::vector<std::string> &Datas, int &PositionOfDatas)
            {
                for (int i = PositionOfDatas; i < Datas.size(); i++)
                {
                    ...
                    if(this->IfDatas.find("==") != -1 || this->IfDatas.find("!=") != -1 || this->IfDatas.find(">=") != -1 || this->IfDatas.find("<=") != -1 || this->IfDatas.find(">") != -1 || this->IfDatas.find("<") != -1){
                        Functions funct;
                        std::cout << funct.returnToF(this->IfDatas) << std::endl;
                    }
